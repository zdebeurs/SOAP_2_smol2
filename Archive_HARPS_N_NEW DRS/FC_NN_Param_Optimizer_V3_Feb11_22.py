...
num_dense_layers_list = [1, 2, 4, 8, 12, 16, 32] #[1, 2, 4, 8, 12, 16]
weight_decay_list = np.geomspace(0.00001, 0.1, 1000) #0.0001, 0.01, 1000)
num_epochs_list = [25, 30, 35, 40, 45, 50, 55, 60]


LOG_DIR_1 = "FC_NN_tensorlogs_Feb11_22/" + time.strftime("%Y%m%d-%H%M%S", time.localtime())
print(LOG_DIR_1)
...
ts = time.time()
for model_num in range(1,500):#30):
  rms_avg_list = []
  weight_decay_list_t = []
  ...
  for index in range(0, len(VAL_FILE_NAME_LIST)):
    ...
    run_dir = LOG_DIR_1 + "individual/{}/split{}/".format(model_num, index)
