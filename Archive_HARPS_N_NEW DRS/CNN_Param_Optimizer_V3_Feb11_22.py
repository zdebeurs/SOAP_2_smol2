...
exp_list = np.random.uniform(1,5, 1000) #(2,4, 1000)
learning_rate_list = [10**-exp_list][0]#[10**-exp_list[0], 10**-exp_list[1], 10**-exp_list[2]]

conv_kernel_size_list = [1,3,5,7,9,11]
num_conv_filters_list = [2, 4, 8, 16, 32, 64]
num_conv_layers_list = [1, 2, 4, 6, 8, 10]
dense_units_list = [50, 100, 200, 500, 1000, 2000]
num_dense_layers_list = [1, 2, 4, 6, 8, 10, 12]
weight_decay_list = np.geomspace(0.00001, 0.1, 1000)#(0.0005, 0.05, 1000)
num_epochs_list = [50, 55, 65, 70, 80, 90, 100,110,120]


LOG_DIR_1 = "CNN_tensorlogs_Feb11_22/" + time.strftime("%Y%m%d-%H%M%S", time.localtime())
#print(LOG_DIR_4)

# Log the hparam config. This is optional but makes tensorboard show the right ranges.
for folder in ['individual', 'aggregate']:
...
for model_num in range(1,1000):#30):
      rms_avg_list = []
      weight_decay_list_t = []
      ...
      for index in range(0, len(VAL_FILE_NAME_LIST)):
        ...
        run_dir = LOG_DIR_1 + "individual/{}/split{}/".format(model_num, index) #"/{}/".format(model_num)
        with tf.summary.create_file_writer(run_dir).as_default():
          ...
          metric_values_list.append([value for metric, value in final_metrics.items()])
          metric_names_list.append([metric for metric, value in final_metrics.items()])

        #print("________________________")
        #print()
      # compute aggregate statistics across crossval runs
      metric_values_transpose = np.array(metric_values_list).transpose()
      crossval_metrics = {"model": model_num}
      for value_num in range(0, len(metric_names_list[0])):
          crossval_metrics[metric_names_list[0][value_num]] = np.mean(metric_values_transpose[value_num])

      # define aggrgate directory and write results to tensorboard
      aggreg_dir = LOG_DIR_1 + "aggregate/{}/".format(model_num)
      with tf.summary.create_file_writer(aggreg_dir).as_default():
          hp.hparams(hparams)  # record the values used in this trial
          # Log metrics to tensorboard.
          for metric, value in crossval_metrics.items():
            tf.summary.scalar(metric, value, step=1)
          final_metrics = crossval_metrics