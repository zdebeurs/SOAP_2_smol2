...
weight_decay_list = np.geomspace(0.0001, 0.01, 1000)
num_epochs_list = [30, 35, 40, 45, 50]


LOG_DIR_1 = "tensorlogs/" + time.strftime("%Y%m%d-%H%M%S", time.localtime())
print(LOG_DIR_1)
...
for i in trange(0,500):
  ...
  with tf.summary.create_file_writer(run_dir).as_default():
      ...
      metric_values = train_cross_val(model, hparams, plots="OFF", model_name = "FC NN",num_epochs=epochs)
