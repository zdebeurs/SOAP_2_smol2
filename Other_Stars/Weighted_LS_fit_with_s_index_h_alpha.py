...
def Weighted_LS_fit_with_h_alpha(ccf_indexes, ccf_list_np, ccf_list_np_transpose, rv_np, s_rv_np):
    # perform the L-S fit --------------
    ccf_len = len(ccf_list_np_transpose)
    n_cols = len(ccf_indexes) + 2
    ...
    # add s-index, h-alpha, BIS, fhwm
    #x[:, n_cols - 3] = ccf_list_np_transpose[ccf_len - 3]
    #x[:, n_cols - 2] = ccf_list_np_transpose[ccf_len - 2]
    ...
    return a_coeff, y_preds, CCF_matrix, og_rms, new_rms

if __name__ == '__main__':
    Weighted_LS_fit_with_s_index_h_alpha()