...
def multi_residual_plot(rv_list, x_axis, ccfs_of_interest, num_ref_ccf, plot_title, numplot):
    # create color scheme
    ...
    fig, ax = plt.subplots(numplot, 1, figsize=(10, 6))
    num = 0
    for i in np.arange(0, len(ccfs_of_interest)):
        if num_ref_ccf == "median":
            ax[numplot].plot(x_axis, ccfs_of_interest[i] - np.median(list(ccfs_of_interest), axis=0), color=colors[num])
        else:
            if i != num_ref_ccf:
                ax[numplot].plot(x_axis, ccfs_of_interest[i] - ccfs_of_interest[num_ref_ccf], color=colors[num])
        num += 1
